...
export const WalletConnect: React.FC<WalletConnectProps> = ({
  ...
  return (
    <div className="obsidian-card p-6 border border-white/[0.08]">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold badge-preprod">
                <ShieldCheck className="w-3 h-3" />
                {network}
              </span>
              {isConnected && connectionType && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono badge-emerald">
                  {connectionType === 'lace' ? 'Live Lace Extension' : 'Sandbox Session'}
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto justify-between sm:justify-end">
          {isConnected ? (
            <div className="flex items-center gap-2.5 w-full sm:w-auto justify-between">
              {/* Balance Badge */}
              <div className="px-3.5 py-1.5 rounded-xl bg-[#040713] border border-white/[0.08] flex items-center gap-2">
              ...
              <button
                ...
                className="btn-pill-secondary text-xs px-4 py-2 text-slate-300 hover:text-rose-400"
              >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className="btn-pill-primary text-xs px-5 py-2.5 disabled:opacity-50 flex-1 sm:flex-initial"
              >
                {isConnecting ? (
                  <>
              ...
              {onConnectSandbox && (
                <button
                  ...
                  className="btn-pill-secondary text-xs px-4 py-2.5 text-slate-200"
      ...
      {isConnected && walletAddress && (
        <div className="mt-4 pt-3.5 border-t border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 bg-[#050914] rounded-xl p-3 border border-white/[0.06]">
          ...
          <button
            onClick={copyAddress}
            className="px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-mono transition-colors flex items-center gap-1 shrink-0 self-start sm:self-auto border border-white/10"
          >
            {copied ? (
              <>
